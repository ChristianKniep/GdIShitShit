...
\multirow{3}{*}{Anw. } & \textcolor{red}{A}way! & 7 & Application & \textcolor{red}{A}lle\\
& \textcolor{red}{P}izza & 6 & Presentation & \textcolor{red}{P}riester \\
& \textcolor{red}{S}alami & 5 & Session  & \textcolor{red}{S}aufen\\
\hline
\multirow{4}{*}{Trans.} & \textcolor{red}{T}hrow & 4 & Transport  & \textcolor{red}{T}equilla\\
& \textcolor{red}{N}ot & 3 & Network  & \textcolor{red}{N}ach \\
& \textcolor{red}{D}o & 2 & Data Link  & \textcolor{red}{D}er\\
& \textcolor{red}{P}lease & 1 & Physical  & \textcolor{red}{P}redigt \\
\end{tabular}
...
Bei der Fragmentation wird ein Paket je nach MTU der n"achsten Verbindung zerlegt.\\
Die geschieht stets in 8Byte Bl"ocken, wird es zerlegt, so wird das More-Flag gesetzt.\\
Ist die MTU also 1500 und sollen 2000 Byte gesendet werden so werden im ersten Block so viele 8ter Bytebl"ocke versendet wie reinpassen und das Offset dementsprechend angepasst.
\begin{tabular}{l|l|l|l|l}
FragNr & Payload (8ter Split) & FragLength & FragOffset & More-Bit \\
1 & 1480 (185*8) & 1500 & 0 & 1\\
2 & 520 (Rest)  & 1500 & 185 & 0
\end{tabular}
...
\begin{itemize}
    \item Gattungsbezeichnung
    \item topologieunabh"angig
    \item Protokolle f"ur unterschiedliche Topos (popul"ar$\Rightarrow$weniger) \\
        RIP,OSPF,BGP,IGRP,Hello
    \item Multiple Protokolle pro AS m"oglich
\end{itemize}

\subsubsection{Exterior Gateway Protocol}
\begin{itemize}
    \item Baut auf IP - Schicht 3
    \item Limitiert auf B"aume
    \item Skaliert nicht gut, daher obsolet
    \item Netzen von Netzen
    \item Jedes Netz min. ein Edge Router
\end{itemize}
...
gelegentlich mit anderen Netzen kommunizieren. 
...
\begin{algorithmic}
	...
	\STATE $d(s) \gets 0$
 	...
 	\FORALL { $u \in S'$ }
 		\STATE $d'(u) = c(s,u)$
 	\ENDFOR
 	\FORALL {$u \in V \setminus  \lbrace S,S' \rbrace$}
...
\begin{verbatim}
01  fuer jedes v aus V                   
02      Distanz(v) := unendlich, Vorg"anger(v) := kein
03  Distanz(s) := 0

04  wiederhole n - 1 mal               
05      fuer jedes (u,v) aus E
06          wenn Distanz(u) + Gewicht(u,v) < Distanz(v)
07          dann
08              Distanz(v) := Distanz(u) + Gewicht(u,v)
09              Vorgaenger(v) := u

10  fuer jedes (u,v) aus E                
11      wenn Distanz(u) + Gewicht(u,v) < Distanz(v) dann
12          STOPP mit Ausgabe 'Kreis negativen Gewichtes.'
